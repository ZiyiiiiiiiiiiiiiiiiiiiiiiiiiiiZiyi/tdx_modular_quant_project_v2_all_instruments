...
<body>
  ...
  <script>
    ...
    function updateReports(exposure,ms,computed){
      ...
      if(String(ms.strategy_logic_version||"")==="mainline_v3_cabinet_native") appendReport("entryGateText",["","Cabinet-native v3",`Strict entry family : ${fmtNum(ms.cabinet_strict_entry_score_mean,3)}`,`Proxy entry family  : ${fmtNum(ms.cabinet_proxy_entry_score_mean,3)}`,`Timing role         : ${fmtNum(ms.cabinet_timing_score_mean,3)}`,`Liquidity health    : ${fmtNum(ms.cabinet_liquidity_health_score_mean,3)}`,`Risk safety         : ${fmtNum(ms.cabinet_risk_safety_score_mean,3)}`,`Hold support        : ${fmtNum(ms.cabinet_hold_support_score_mean,3)}`]);
