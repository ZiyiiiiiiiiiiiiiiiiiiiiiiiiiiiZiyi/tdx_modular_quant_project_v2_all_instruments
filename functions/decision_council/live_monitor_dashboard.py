...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .product-links { display: flex; gap: 7px; margin-top: 7px; }
    .product-link { color: #eaf4ef; text-decoration: none; border: 1px solid #52645f; border-radius: 4px; padding: 4px 8px; font-size: 11px; }
    .product-link:hover { background: #344640; }
...
<body>
  ...
  <main class="workspace">
    ...
    <section class="tab-panel" id="tab-risk">
      ...
      <article class="panel" style="margin-top:12px">
        <div class="panel-header"><div><div class="panel-title">持仓数量路径</div><div class="panel-note">实际、优化器计划与政策最低/目标/上限；沿用总览的60/180/全部区间</div></div></div>
        <div class="chart-body"><canvas class="chart medium" id="holdingCountChart"></canvas></div>
        <div class="panel-body legend">
          <span class="legend-item"><i class="legend-line" style="background:#087a55"></i>实际持仓</span>
          <span class="legend-item"><i class="legend-line" style="background:#246b9e"></i>优化器计划</span>
          <span class="legend-item"><i class="legend-line" style="background:#bd3d39"></i>政策最低</span>
          <span class="legend-item"><i class="legend-line" style="background:#a87518"></i>政策目标</span>
          <span class="legend-item"><i class="legend-line" style="background:#d17b2f"></i>可执行目标</span>
          <span class="legend-item"><i class="legend-line" style="background:#3c8d9d"></i>权限可达</span>
          <span class="legend-item"><i class="legend-line" style="background:#705a9f"></i>政策上限</span>
        </div>
      </article>
      <article class="panel" style="margin-top:12px">
        <div class="panel-header"><div><div class="panel-title">仓位暴露合同路径</div><div class="panel-note">政策下界/目标、可执行目标、权限可达、计划、实际和硬上限；0为真实值，缺失不回填</div></div></div>
        <div class="chart-body"><canvas class="chart medium" id="exposureContractChart"></canvas></div>
        <div class="panel-body legend">
          <span class="legend-item"><i class="legend-line" style="background:#bd3d39"></i>政策下界</span>
          <span class="legend-item"><i class="legend-line" style="background:#a87518"></i>政策目标</span>
          <span class="legend-item"><i class="legend-line" style="background:#d17b2f"></i>可执行目标</span>
          <span class="legend-item"><i class="legend-line" style="background:#3c8d9d"></i>权限可达</span>
          <span class="legend-item"><i class="legend-line" style="background:#246b9e"></i>优化器计划</span>
          <span class="legend-item"><i class="legend-line" style="background:#087a55"></i>实际</span>
          <span class="legend-item"><i class="legend-line" style="background:#705a9f"></i>硬上限</span>
        </div>
      </article>
  ...
  <script>
    const KPI_DEFS = [
      ...
      ["strategic_exposure_budget", "战略暴露预算", "SCAP资金治理"], ["signal_supported_exposure", "信号支持暴露", "候选证据"],
      ["integer_feasible_exposure", "整数可行暴露", "整手与现金"], ["planned_exposure", "计划暴露", "ActionPlan"],
      ["selected_position_count", "计划名称数", "ActionPlan"], ["incremental_expected_wealth_amount", "增量期望财富", "人民币"],
      ["incremental_cvar_amount", "增量CVaR", "人民币"], ["model_uncertainty_amount", "模型不确定性", "人民币"],
      ["scenario_risk_penalty_amount", "情景风险罚金", "人民币"], ["best_rejected_objective_amount", "最佳被拒目标", "人民币"],
    ];
    const DETAIL_GROUPS = {
      overviewDetails: [
      ...
      riskDetails: [
        ...
        ["executable_target_holding_count","可执行目标持仓"],["authority_attainable_holding_count","权限可达持仓"],
        ["executable_target_exposure","可执行目标暴露"],["authority_attainable_exposure","权限可达暴露"],
        ["policy_floor_feasible_after_authority","权限后下界可行"],["plan_floor_contract_violation","计划合同违约"],
    ...
    function drawHoldingCount() { const c=prepareCanvas("holdingCountChart"),points=visibleHistory();if(!c)return;const {ctx,w,h}=c;if(!points.length)return drawEmpty(ctx,"等待持仓数量数据");const actual=points.map(x=>nullableNumber(x.actualHoldingCount)),planned=points.map(x=>nullableNumber(x.plannedHoldingCount)),minimum=points.map(x=>nullableNumber(x.minimumHoldingCount)),target=points.map(x=>nullableNumber(x.targetHoldingCount)),executable=points.map(x=>nullableNumber(x.executableTargetHoldingCount)),attainable=points.map(x=>nullableNumber(x.authorityAttainableHoldingCount)),maximum=points.map(x=>nullableNumber(x.maximumHoldingCount));const observed=[...actual,...planned,...minimum,...target,...executable,...attainable,...maximum].filter(x=>x!==null);if(!observed.length)return drawEmpty(ctx,"历史状态没有持仓数量字段");const bounds={min:Math.min(0,...observed)-.5,max:Math.max(...observed)+.5},p=drawFrame(ctx,w,h,bounds,points.map(x=>x.date),x=>fmtNum(x,0));drawStepLine(ctx,actual,p,bounds,"#087a55",2.6);drawStepLine(ctx,planned,p,bounds,"#246b9e",1.8);drawStepLine(ctx,minimum,p,bounds,"#bd3d39",1.2);drawStepLine(ctx,target,p,bounds,"#a87518",1.4);drawStepLine(ctx,executable,p,bounds,"#d17b2f",1.4);drawStepLine(ctx,attainable,p,bounds,"#3c8d9d",1.4);drawStepLine(ctx,maximum,p,bounds,"#705a9f",1.2);const show=x=>x===null?"--":fmtNum(x,0);drawHover(ctx,p,points,[{label:"实际",values:actual,format:show},{label:"计划",values:planned,format:show},{label:"最低",values:minimum,format:show},{label:"政策目标",values:target,format:show},{label:"可执行",values:executable,format:show},{label:"权限可达",values:attainable,format:show},{label:"上限",values:maximum,format:show}]); }
    function drawExposureContract() { const c=prepareCanvas("exposureContractChart"),points=visibleHistory();if(!c)return;const {ctx,w,h}=c;if(!points.length)return drawEmpty(ctx,"等待仓位合同数据");const lower=points.map(x=>nullableNumber(x.policyExposureLower)),target=points.map(x=>nullableNumber(x.policyExposureTarget)),executable=points.map(x=>nullableNumber(x.executableTargetExposure)),attainable=points.map(x=>nullableNumber(x.authorityAttainableExposure)),planned=points.map(x=>nullableNumber(x.plannedExposure)),actual=points.map(x=>nullableNumber(x.actualExposure)),hard=points.map(x=>nullableNumber(x.hardExposureCeiling));const observed=[...lower,...target,...executable,...attainable,...planned,...actual,...hard].filter(x=>x!==null);if(!observed.length)return drawEmpty(ctx,"历史状态没有仓位合同字段");const bounds={min:Math.max(Math.min(0,...observed)-.03,0),max:Math.min(Math.max(...observed)+.03,1.03)},p=drawFrame(ctx,w,h,bounds,points.map(x=>x.date),fmtPct);drawStepLine(ctx,lower,p,bounds,"#bd3d39",1.2);drawStepLine(ctx,target,p,bounds,"#a87518",1.4);drawStepLine(ctx,executable,p,bounds,"#d17b2f",1.4);drawStepLine(ctx,attainable,p,bounds,"#3c8d9d",1.4);drawStepLine(ctx,planned,p,bounds,"#246b9e",1.8);drawStepLine(ctx,actual,p,bounds,"#087a55",2.6);drawStepLine(ctx,hard,p,bounds,"#705a9f",1.2);const show=x=>x===null?"--":fmtPct(x);drawHover(ctx,p,points,[{label:"政策下界",values:lower,format:show},{label:"政策目标",values:target,format:show},{label:"可执行",values:executable,format:show},{label:"权限可达",values:attainable,format:show},{label:"计划",values:planned,format:show},{label:"实际",values:actual,format:show},{label:"硬上限",values:hard,format:show}]); }
    ...
    function drawAllCharts(){drawPerformance();drawExcess();drawDrawdown();drawHoldingCount();drawExposureContract();drawMulti("factorChart",factorHistory,"model_name","因子权重");drawMulti("moduleChart",moduleHistory,"factor_module","模块权重");drawHoldingPaths(window.latestHoldingPaths||[]);}
    ...
    function updateReports(exposure,ms,computed){
      ...
      document.getElementById("safetyText").textContent=[`风险等级              : ${ms.risk_level||"--"}`,`原始风险等级          : ${ms.raw_risk_level||"--"}`,`触发连续天数          : ${ms.trigger_streak_days??"--"}`,`触发来源              : ${ms.trigger_source||"--"}`,`安全结构市场状态      : ${ms.safety_structural_state||ms.structural_regime_level||"--"}`,`安全状态启用          : ${Boolean(ms.safety_market_state_active)}`,`安全状态权限          : ${ms.safety_market_state_authority||"--"}`,`状态仓位预算          : ${fmtPct(finite(ms.regime_exposure_budget))}`,`安全仓位上限          : ${fmtPct(finite(ms.safety_exposure_cap))}`,`硬冻结                : ${Boolean(ms.hard_freeze_active)}`,`允许追仓              : ${Boolean(ms.catchup_allowed)}`,`准确率乘数            : ${fmtNum(ms.accuracy_multiplier,2)}`,`运行状态              : ${ms.regime||"--"}`,`最大持股数            : ${ms.top_n??"--"}`].join("\n");
      ...
      appendReport("exposureText",[`仓位合同版本          : ${ms.sizing_contract_version??"历史合同无此字段"}`,`仓位合同ID            : ${String(ms.sizing_contract_id||"--").slice(0,32)}`,`政策持仓最低/目标     : ${ms.minimum_required_holding_count??"--"} / ${ms.soft_target_holding_count??"--"}`,`可执行/权限可达持仓   : ${ms.executable_target_holding_count??"--"} / ${ms.authority_attainable_holding_count??"--"}`,`计划/实际持仓         : ${ms.planned_holding_count??ms.optimizer_planned_holding_count??"--"} / ${exposure.holding_count??"--"}`,`政策暴露下界/目标     : ${fmtPct(finite(ms.policy_exposure_lower,NaN))} / ${fmtPct(finite(ms.policy_exposure_target,NaN))}`,`可执行/权限可达暴露   : ${fmtPct(finite(ms.executable_target_exposure,NaN))} / ${fmtPct(finite(ms.authority_attainable_exposure,NaN))}`,`计划/实际暴露         : ${fmtPct(finite(ms.optimizer_planned_exposure,NaN))} / ${fmtPct(finite(ms.actual_exposure,NaN))}`,`权限后下界可行        : ${ms.policy_floor_feasible_after_authority===undefined?"历史合同无此字段":String(Boolean(ms.policy_floor_feasible_after_authority))}`,`结构短缺原因          : ${ms.structural_floor_shortfall_reasons||"--"}`,`计划合同违约          : ${ms.plan_floor_contract_violation===undefined?"--":String(Boolean(ms.plan_floor_contract_violation))}`,`单名称基础目标金额    : ${fmtMoney(finite(ms.sizing_base_new_name_target_amount,NaN))}`,`交易硬容量上限        : ${ms.economic_position_cap??"--"}`,`旧分仓参考（影子）    : ${ms.sizing_reference_positions??"历史合同无此字段"}`,`基础状态仓位          : ${fmtPct(finite(ms.base_exposure_by_regime))}`,`原始安全上限          : ${fmtPct(finite(ms.raw_safety_exposure_cap))}`,`选中动作涉及名称数    : ${ms.selected_position_count??"--"}`,`盈利覆盖比/概率下界   : ${fmtNum(finite(ms.profit_coverage_ratio),2)} / ${fmtPct(finite(ms.profit_coverage_probability_lower))}`,`覆盖证据股票数        : ${ms.coverage_evidence_name_count??0}`,`原始正效用信号        : ${ms.raw_signal_count??0}`,`结构可行              : ${ms.structural_feasible_count??0}`,`现金可行              : ${ms.cash_feasible_count??0}`,`槽位可行              : ${ms.slot_feasible_count??0}`,`优化器选中            : ${ms.optimizer_selected_entry_count??0}`,`计划安全卖出          : ${fmtPct(finite(ms.planned_safety_sell_weight))}`,`约束现金保留          : ${fmtPct(finite(ms.constraint_cash_reserve))}`,`普通换手权重          : ${fmtPct(finite(ms.normal_turnover_weight))}`,`目标总漂移            : ${fmtPct(finite(ms.total_target_drift))}`,`追仓预算              : ${fmtPct(finite(ms.catchup_buy_budget))}`,`追仓档位              : ${ms.catchup_tier||"--"}`,`退出阶段/止损         : ${ms.scap_exit_stage||"--"} / ${fmtPct(finite(ms.scap_loss_stop))}`,`运行身份              : ${String(ms.runtime_identity_hash||"--").slice(0,16)}`]);
      ...
      appendReport("exposureText",[`覆盖模式              : ${ms.coverage_mode||"--"}`,`增量期望/CVaR         : ${fmtMoney(finite(ms.incremental_expected_wealth_amount))} / ${fmtMoney(finite(ms.incremental_cvar_amount))}`,`模型不确定性/风险罚金 : ${fmtMoney(finite(ms.model_uncertainty_amount))} / ${fmtMoney(finite(ms.scenario_risk_penalty_amount))}`,`情景证据状态          : ${ms.scenario_evidence_state||"--"}`,`最佳被拒目标          : ${fmtMoney(finite(ms.best_rejected_objective_amount))}`,
        "",`战略暴露预算          : ${fmtPct(finite(ms.strategic_exposure_budget,NaN))}`,`信号支持暴露          : ${fmtPct(finite(ms.signal_supported_exposure,NaN))}`,`整数可行暴露          : ${fmtPct(finite(ms.integer_feasible_exposure,NaN))}`,`计划暴露              : ${fmtPct(finite(ms.planned_exposure,NaN))}`,`用户持仓硬上限        : ${ms.user_hard_position_cap??"--"}`,`成本可行持仓上限      : ${ms.cost_feasible_position_cap??"--"}`,`整手现金持仓上限      : ${ms.lot_cash_position_cap??"--"}`,`风险可行持仓上限      : ${ms.risk_feasible_position_cap??"--"}`,`搜索持仓上限          : ${ms.search_position_cap??"--"}`,`有效持仓上限          : ${ms.effective_position_cap??"--"}`,`超额沿用持仓          : ${ms.grandfathered_excess_names??0}`,`每日新增名称上限      : ${ms.max_daily_new_names??"--"}`,`每日新增暴露上限      : ${fmtPct(finite(ms.max_daily_new_exposure_ratio,NaN))}`,`组合常规再平衡频率    : ${ms.portfolio_normal_rebalance_frequency||"--"}`,`组合再平衡锚点        : ${ms.portfolio_normal_rebalance_anchor||"--"}`,`月度计划执行窗口      : ${ms.monthly_plan_execution_window_sessions??"--"}`]);
    }
    ...
    function hydrateChartHistory(rows){
      ...
      for(const row of rows||[]){
        ...
        restored.push({key:`${dayIndex}|${date}`,date,nav,navMultiple,benchmarkNav,excessNav,drawdown:peak>0?nav/peak-1:0,cash:finite(row.cash),invested:finite(row.invested_value),actualExposure,validInvestedNav,actualHoldingCount:nullableNumber(row.actual_holding_count),plannedHoldingCount:nullableNumber(row.optimizer_planned_holding_count),minimumHoldingCount:nullableNumber(row.minimum_required_holding_count),targetHoldingCount:nullableNumber(row.soft_target_holding_count),executableTargetHoldingCount:nullableNumber(row.executable_target_holding_count),authorityAttainableHoldingCount:nullableNumber(row.authority_attainable_holding_count),maximumHoldingCount:nullableNumber(row.maximum_allowed_holding_count),policyExposureLower:nullableNumber(row.policy_exposure_lower),policyExposureTarget:nullableNumber(row.policy_exposure_target),executableTargetExposure:nullableNumber(row.executable_target_exposure),authorityAttainableExposure:nullableNumber(row.authority_attainable_exposure),plannedExposure:nullableNumber(row.optimizer_planned_exposure),hardExposureCeiling:nullableNumber(row.daily_effective_exposure_ceiling),sizingContractId:String(row.sizing_contract_id||""),floorFeasibleAfterAuthority:row.policy_floor_feasible_after_authority??null,shortfallReasons:String(row.structural_floor_shortfall_reasons||""),policyBandState:String(row.policy_band_state||"")});
        previousNav=nav;
      }
    ...
    function renderState(payload){
      ...
      const currentPoint={key:pointKey,date:dateKey,nav,navMultiple,benchmarkNav,excessNav,drawdown,cash:finite(exposure.cash),invested:finite(exposure.invested_value),actualExposure,validInvestedNav:previousInvested*(1+investedRet),actualHoldingCount:nullableNumber(exposure.holding_count),plannedHoldingCount:nullableNumber(ms.optimizer_planned_holding_count??ms.planned_holding_count),minimumHoldingCount:nullableNumber(ms.minimum_required_holding_count),targetHoldingCount:nullableNumber(ms.soft_target_holding_count),executableTargetHoldingCount:nullableNumber(ms.executable_target_holding_count),authorityAttainableHoldingCount:nullableNumber(ms.authority_attainable_holding_count),maximumHoldingCount:nullableNumber(ms.maximum_allowed_holding_count),policyExposureLower:nullableNumber(ms.policy_exposure_lower),policyExposureTarget:nullableNumber(ms.policy_exposure_target),executableTargetExposure:nullableNumber(ms.executable_target_exposure),authorityAttainableExposure:nullableNumber(ms.authority_attainable_exposure),plannedExposure:nullableNumber(ms.optimizer_planned_exposure??ms.planned_exposure),hardExposureCeiling:nullableNumber(ms.daily_effective_exposure_ceiling),sizingContractId:String(ms.sizing_contract_id||""),floorFeasibleAfterAuthority:ms.policy_floor_feasible_after_authority??null,shortfallReasons:String(ms.structural_floor_shortfall_reasons||""),policyBandState:String(ms.policy_band_state||"")};if(hasCurrent)history[history.length-1]=currentPoint;else history.push(currentPoint);if(history.length>1200)history=history.slice(-1200);
      ...
      const parityMetrics={
        ...
        scap_v31_all_d_streak:[String(ms.scap_v31_all_d_streak??0),-finite(ms.scap_v31_all_d_streak)],
        scap_v31_normal_cash_zero_proposal_streak:[String(ms.scap_v31_normal_cash_zero_proposal_streak??0),-finite(ms.scap_v31_normal_cash_zero_proposal_streak)],
