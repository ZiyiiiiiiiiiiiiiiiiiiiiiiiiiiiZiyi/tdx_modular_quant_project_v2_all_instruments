...
def append_rsi_timing_factors(df: pd.DataFrame, *, close_col: str = "close_nominal") -> pd.DataFrame:
    ...
    rsi_low_10 = grouped_apply(rsi, frame, 10, "min")
    ...
    frame["rsi_recovery_14"] = ((rsi_prev <= 35.0) & (rsi > rsi_prev) & (rsi >= 30.0)).astype(float) * (rsi - rsi_prev) / 100.0
    frame["rsi_overheat_14"] = -((rsi - 75.0).clip(lower=0.0) / 25.0)
    frame["rsi_pullback_in_uptrend"] = ((ma20 > ma60) & (rsi < 55.0) & (rsi >= 40.0)).astype(float) * (55.0 - rsi).clip(lower=0.0) / 55.0
    price_new_low = close <= close_low_20
    rsi_not_new_low = rsi > rsi_low_20
    frame["rsi_divergence_proxy"] = (price_new_low & rsi_not_new_low).astype(float) * (rsi - rsi_low_20).clip(lower=0.0) / 100.0
    frame["rsi_slope_5"] = (rsi - grouped_apply(rsi, frame, 5, "shift")) / 5.0
    frame["rsi_percentile_252"] = grouped_apply(rsi, frame, 252, "rank_pct")
    return frame
