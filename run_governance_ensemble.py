# -*- coding: utf-8 -*-
"""
Governance + Ensemble (LightGBM + TabNet) Version
...
def run_backtest(gui):
    ...
    try:
        ...
        gui.root.after(0, lambda: gui.update_progress(0, total_days, "Training Ensemble...", f"Processing {total_days} trading days"))
        
        # Initialize both models
        lgbm_model = LightGBMAlphaModel(lookback_days=500)
        tabnet_model = TabNetAlphaModel(lookback_days=500)
        ...
        for day_index, date in enumerate(dates):
            ...
            # Predict with ensemble (60% LightGBM, 40% TabNet)
            if lgbm_model.is_fitted or tabnet_model.is_fitted:
                X_daily, _ = prepare_ml_features(daily, available_features)
                
                lgbm_pred = lgbm_model.predict(X_daily) if lgbm_model.is_fitted else np.zeros(len(daily))
                tabnet_pred = tabnet_model.predict(X_daily) if tabnet_model.is_fitted else np.zeros(len(daily))
                
                # Ensemble: 60% LightGBM + 40% TabNet
                ensemble_pred = 0.6 * lgbm_pred + 0.4 * tabnet_pred
                mask = features["date"] == pd.Timestamp(date)
                features.loc[mask, "score_ml_alpha"] = ensemble_pred.astype(np.float32)
